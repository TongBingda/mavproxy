...
def swarm_task():
    ...
    while True:
        ...
        if current_mode == 'GUIDED':
            ...
            continue
    # this vehicle is the first vehicle in the swarm
    # send data to the second vehicle
    if thisid == '\x01':
        ...
        try:
            ...
            if rec_msg[3] == thisid or rec_msg[3] == '\x00':
                if rec_msg[4] == "\x00": # do nothing
                    ...
                    current_mode = mpstate.status.flightmode
                    if current_mode == 'GUIDED':
                        cmd_speed = struct.unpack('f', rec_msg[21:25])[0]
                        process_stdin('setspeed ' + str(cmd_speed))
                        time.sleep(0.5)
