...
# open RFD900x telem serial port, revised by tongbingda
telem = serial.Serial("/dev/ttyUSBRFD900x", 115200)
...
# swarm_task(), revised by tongbingda
def swarm_task():
    # get current sysid and pack it into a char
    # this vehicle's ID is pre-defined as 0x02 
    thisid = '\x02'
    # set zhe vehicle mode to fbwa or takeoff
    # the second loop will not run
    while True:
        current_mode = mpstate.status.flightmode
        if current_mode == 'GUIDED':
            break
        else:
            continue
    # if this vehicle is the first vehicle in the swarm
    # then send data to the second vehicle
    if thisid == '\x01':
        status_send(thisid, '\x02', '\x0E')
        time.sleep(0.5)
    # duty loop
    while True:
        try:
            # protocol version 1.1, read 26 bytes at once
            rec_msg = telem.read(26)
            if rec_msg[3] == thisid or rec_msg[3] == '\x00':
                if rec_msg[4] == '\x00': # do nothing
                    continue
                elif rec_msg[4] == '\x01': # arm throttle
                    # arm this vehicle
                    process_stdin("arm throttle")
                elif rec_msg[4] == '\x02': # disarm
                    # disarm this vehicle
                    process_stdin('disarm')
                elif rec_msg[4] == '\x03': # arm safetyon
                    # enable the safety button
                    process_stdin('arm safetyon')
                elif rec_msg[4] == '\x04': # arm safetyoff
                    # disable the safety button
                    process_stdin('arm safetyoff')
                elif rec_msg[4] == '\x05': # arm uncheck all
                    # disable arming checks if encounter some difficult problems
                    process_stdin('arm uncheck all')
                elif rec_msg[4] == '\x06': # mode auto
                    # mode change to auto
                    process_stdin('mode auto')
                elif rec_msg[4] == '\x07': # mode loiter
                    # mode change to loiter
                    process_stdin('mode loiter')
                elif rec_msg[4] == '\x08': # mode rtl
                    # mode change to RTL
                    process_stdin('mode RTL')
                elif rec_msg[4] == '\x09': # mode fbwa
                    # mode change to FBWA
                    process_stdin('mode FBWA')
                elif rec_msg[4] == '\x0A': # mode TAKEOFF
                    # mode change to takeoff
                    process_stdin('mode TAKEOFF')
                elif rec_msg[4] == '\x0B': # mode guided
                    # mode change to guided
                    process_stdin('mode GUIDED')
                elif rec_msg[4] == '\x0C': # setspeed
                    # set this vehicle's speed
                    current_mode = mpstate.status.flightmode
                    if current_mode == 'GUIDED':
                        cmd_spd = struct.unpack('f', rec_msg[21:25])[0]
                        process_stdin('setspeed ' + str(cmd_spd))
                elif rec_msg[4] == '\x0D': # changealt
                    # set this vehicle's altitude
                    current_mode = mpstate.status.flightmode
                    if current_mode == 'GUIDED':
                        cmd_alt = struct.unpack('f', rec_msg[13:17])[0]
                        process_stdin('changealt ' + str(cmd_alt))
                elif rec_msg[4] == '\x0E': # guided
                    # guided to a waypoint, use lat lng alt
                    current_mode = mpstate.status.flightmode
                    if current_mode == 'GUIDED':
                        cmd_lat = struct.unpack('f', rec_msg[5:9])[0]
                        cmd_lng = struct.unpack('f', rec_msg[9:13])[0]
                        cmd_alt = struct.unpack('f', rec_msg[13:17])[0]
                        process_stdin('guided ' + str(cmd_lat) + ' ' + str(cmd_lng) + ' ' + str(cmd_alt))
                elif rec_msg[4] == '\x0F': # swarm control rule for follower vehicle
                    # define desire distance in meters
                    desire_distance = 10
                    # pre define the max_spd and the min_spd, use missionplanner to get these two value
                    max_spd = 20
                    min_spd = 10
                    current_mode =mpstate.status.flightmode
                    if current_mode == 'GUIDED':
                        ...
                        cmd_lng = struct.unpack('f', rec_msg[9:13])[0]
                        cmd_alt = struct.unpack('f', rec_msg[13:17])[0]
                        process_stdin('guided ' + str(cmd_lat) + ' ' + str(cmd_lng) + ' ' + str(cmd_alt -3)) # follower vehicle's altitude is lower than leader vehicle
                        time.sleep(0.5)
                        cmd_spd = struct.unpack('f', rec_msg[21:25])[0]
                        # get distance between command position and local position
                        distance = calculate_distance(cmd_lat, cmd_lng)
                        # use linear method to control vehicle's speed (need to test in air)
                        desire_spd = (distance - desire_distance) * 0.2 + cmd_spd
                        if desire_spd > max_spd:
                            desire_spd = max_spd
                        elif desire_spd < min_spd:
                            desire_spd = min_spd
                        process_stdin('setspeed ' + str(desire_spd))
                elif rec_msg[4] == '\xFF': # print debug message
                    # debug message
                    # pre-defined frame data
                    head = '\xAF\xAF'
                    sysid = mpstate.settings.target_system
                    recid = '\x00'
                    cmd = '\xFF'
                    ...
                    for mav in mpstate.mav_master:
                        if not sysid in mav.sysid_state:
                            continue
                        lat = mav.sysid_state[sysid].messages['GLOBAL_POSITION_INT'].lat * 1.0e-7
                        lng = mav.sysid_state[sysid].messages['GLOBAL_POSITION_INT'].lon * 1.0e-7
                        alt = mav.sysid_state[sysid].messages['VFR_HUD'].alt
                        hdg = mav.sysid_state[sysid].messages['VFR_HUD'].heading
                        spd = mav.sysid_state[sysid].messages['VFR_HUD'].airspeed
                    print('lat = %f' % lat)
                    print('lng = %f' % lng)
                    print('alt = %f' % alt)
                    print('hdg = %d' % hdg)
                    print('spd = %f' % spd)
                    lat_pak = struct.pack('f', lat)
                    lng_pak = struct.pack('f', lng)
                    alt_pak = struct.pack('f', alt)
                    hdg_pak = struct.pack('i', hdg)
                    spd_pak = struct.pack('f', spd)
                    send_msg = head + thisid + recid + cmd + lat_pak + lng_pak + alt_pak + hdg_pak + spd_pak + chk
                    # telem.write(send_msg)
                else:
                    print('Error: Unknown command.')
            time.sleep(0.5)
            status_send(thisid, '\x01', '\x00')
        except e:
            print('Error: %s' % e)
            continue

# status_send(), revised by tongbingda
# protocol version 1.1
# get protocol reference for more information
def status_send(thisid, recid, cmd):
    head = '\xAF\xAF'
    sysid = mpstate.settings.target_system
    chk = "\xFF"
    for mav in mpstate.mav_master:
        ...
        hdg = mav.sysid_state[sysid].messages['VFR_HUD'].heading
        spd = mav.sysid_state[sysid].messages['VFR_HUD'].airspeed
    lat_pak = struct.pack('i', lat)
    lng_pak = struct.pack('i', lng)
    alt_pak = struct.pack('f', alt)
    hdg_pak = struct.pack('i', hdg)
    spd_pak = struct.pack('f', spd)
    send_msg = head + thisid + recid + cmd + lat_pak + lng_pak + alt_pak + hdg_pak + spd_pak + chk
...
def calculate_distance(cmd_lat, cmd_lng):
    earth_radius = 6371.393
    sysid = mpstate.settings.target_system
    for mav in mpstate.mav_master:
        if not sysid in mav.sysid_state:
            continue
        # get local position in degrees
        loc_lat = mav.sysid_state[sysid].messages['GLOBAL_POSITION_INT'].lat * 1.0e-7
        loc_lng = mav.sysid_state[sysid].messages['GLOBAL_POSITION_INT'].lon * 1.0e-7
...
if __name__ == '__main__':
    ...
    # run swarm task as a thread, revised by tongbingda
    swarm_thread = threading.Thread(target=swarm_task, name='swarm_task')
    swarm_thread.deamon = True
    swarm_thread.start()
